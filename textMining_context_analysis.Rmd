...
reviews_tbl <- reviews %>% 
    tbl_df() %>%
    mutate(review_new = str_replace_all(string = review,                        
...
* Following graph indicate users are talking about:  
    + game play (e.g. "反破防", "地行杀", "etc")  
...
word_correlation %>%
    ...
    theme_void()
```
*The correlation is measure by [Phi Coefficient](https://en.wikipedia.org/wiki/Phi_coefficient)*
...
* 4 topics were clustered  
    1.Kind of chitchat  
    2.chinese faction and heroes  
    3.Sales related stuff  
    4.game play strategy related  
